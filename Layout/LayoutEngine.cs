...
namespace Apex.Layout
{
    public class LayoutEngine
    {
        ...
        /// <summary>
        /// The margin to not use inside the container.
        /// </summary>
        ...
        /// <param name="ContainerType">The type of the container to apply margin presets for.</param>
        public LayoutEngine(Control Container, ContainerType ContainerPreset = ContainerType.Generic)
        {
            ...
            switch (ContainerPreset)
            {
                ...
                case ContainerType.GroupBox:
                    {
                        Margin = new Padding(6, 18, 0, 0);
                        break;
                    }
        ...
        /// <summary>
        /// Adds a panel and starts layout inside it.
        /// </summary>
        /// <param name="Panel">The panel to add.</param>
        /// <returns>A LayoutEndStub to facilitate using (BeginPanel()) { } usage.</returns>
        public LayoutEndStub BeginPanel(Panel Panel)
        {
            LayoutItems.Add(new LayoutItem { Control = Panel, ItemType = LayoutItem.Type.BeginPanel });
            return new LayoutEndStub(this, LayoutEndStub.StubType.Panel);
        }

        /// <summary>
        /// Ends the current panel and continues layout one level higher.
        /// </summary>
        public void EndPanel()
        {
            LayoutItems.Add(new LayoutItem { ItemType = LayoutItem.Type.EndPanel });
        }
        ...
        public void ProcessLayout()
        {
            ...
            {
                Width = Container.DisplayRectangle.Width - Margin.Horizontal,
                Height = Container.DisplayRectangle.Height - Margin.Vertical,
                XOffset = Margin.Left,
                YOffset = Margin.Top,
            ...
            for (; index < LayoutItems.Count; index++)
            {
                switch (LayoutItems[index].ItemType)
                {
                    ...
                    case LayoutItem.Type.BeginPanel:
                        {
                            ProcessPanel(ref index, ref currentState);
                            break;
                        }
        ...
        private void ProcessGroupBox(ref int Index, ref LayoutState CurrentState)
        {
            ...
            for (; Index < LayoutItems.Count; Index++)
            {
                ...
                switch (LayoutItems[Index].ItemType)
                {
                    ...
                    case LayoutItem.Type.BeginPanel:
                        {
                            ProcessPanel(ref Index, ref groupBoxState);
                            break;
                        }
                    case LayoutItem.Type.Anchor:
                        {
        ...
        private void ProcessPanel(ref int Index, ref LayoutState CurrentState)
        {
            var panel = (Panel)LayoutItems[Index].Control;
            panel.Left = CurrentState.XOffset;
            panel.Top = CurrentState.YOffset;
            panel.Width = CurrentState.Width;
            panel.Height = 50;
            CurrentState.Controls.Add(panel);
            Index++;
            ...
            // Create a new state for the children of the group box.
            var PanelState = new LayoutState
            {
                Width = panel.ClientSize.Width - Padding.Horizontal - 2,
                Height = panel.ClientSize.Height - Padding.Vertical - 2,
                XOffset = Padding.Left + 1,
            ...
            {
                // Finalise group box and yield control to parent item.
                if (LayoutItems[Index].ItemType == LayoutItem.Type.EndPanel)
                {
                    break;
                }
                switch (LayoutItems[Index].ItemType)
                {
                    case LayoutItem.Type.BeginRow:
                        {
                            ProcessRow(ref Index, ref PanelState);
                            break;
                        }
                    case LayoutItem.Type.Control:
                        {
                            ProcessControl(ref Index, ref PanelState);
                            break;
                        }
                    case LayoutItem.Type.BeginGroupBox:
                        {
                            ProcessGroupBox(ref Index, ref PanelState);
                            break;
                        }
                    case LayoutItem.Type.BeginPanel:
                        {
                            ProcessPanel(ref Index, ref PanelState);
                            break;
                        }
                    case LayoutItem.Type.Anchor:
                        {
                            PanelState.Anchor = (Anchor)LayoutItems[Index].Data;
                            break;
                        }
                    case LayoutItem.Type.OffsetX:
                        {
                            PanelState.XOffset += (int)LayoutItems[Index].Data;
                            break;
                        }
                    case LayoutItem.Type.OffsetY:
                        {
                            PanelState.YOffset += (int)LayoutItems[Index].Data;
                            break;
                        }
                    case LayoutItem.Type.EndGroupBox:
                    case LayoutItem.Type.EndRow:
            ...
            // Notify the parent of the changes to the state.
            panel.Height = PanelState.YOffset + 7 - PanelState.LastYSpacing;
            CurrentState.YOffset += panel.Height + Padding.Horizontal;
            CurrentState.LastYSpacing = Padding.Horizontal;
            CurrentState.LastXSpacing = 0;
        }
                    ...
                    case LayoutItem.Type.BeginPanel:
                        {
                            ...
                            switch (LayoutItems[Index].ItemType)
                            {
                                ...
                                case LayoutItem.Type.BeginPanel:
                                    {
                                        ProcessPanel(ref Index, ref columnState);
                                        break;
                                    }
                                case LayoutItem.Type.BeginRow:
                                    {
